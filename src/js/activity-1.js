...
$(".ff_lightbox_link").click(function(){
    ...
    $.ajax({
        ...
        success: function(data, textStatus, jqXHR) {
            ...
            console.log(data);
            $(".aa_banner").css({
                'background':'url('+data[0].act_img+')'
            });
            $("#a-lb-act_name").text(data[0].act_name);
            $("#a-lb-act_info").text(data[0].act_info);
            $("#a-lb-act_date").text(data[0].act_startDate+"~"+data[0].act_startDate);
            $("#a-lb-actCla_name").text(data[0].actCla_name);
            $("#a-lb-act_place").text(data[0].act_place);
            $("#a-lb-act_price").text(data[0].act_price);


            // for(var i=0;i<=)
            $("#a-comm").append(
                '<div class="comment ct1"><div class="user"><div class="user-pic"><img src="img/i-member2.png">'+
                '</div><div class="user-info"><span><a href="">Kattie918</a></span></div></div>'+'<div class="ct-content">'+
                '<p>參加活動：2015年墾丁南十字觀星攝影<span class="ct-stars"><i class="fa fa-star" aria-hidden="true"></i>'+
